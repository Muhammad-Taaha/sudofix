...
def test_overflow(binary):
    print("[*] Testing VULN-1: integer overflow in allocation")
    config = {
        "port": 18080,
        "jobs": [{
            "type": "transform",
...
def test_uaf(binary):
    print("[*] Testing VULN-2: use-after-free (direct buffer)")
    config = {
        "port": 18081,
        "jobs": [{
            "type": "data_manage",
...
def test_path_traversal(binary):
    print("[*] Testing VULN-3: path traversal via symlink")
...
def test_race(binary):
    print("[*] Testing VULN-4: race condition on pool")
    config = {
        "port": 18083,
        "jobs": [{
            "type": "transform",
...
def test_double_close(binary):
    print("[*] Testing VULN-6: double close / resource leak")
    ...
    try:
        ...
        if os.path.exists(leak_file):
            with open(leak_file, 'rb') as f:
                data = f.read()
            # look for non-zero bytes beyond the written part
            if len(data) > 10 and any(b != 0 for b in data[5:]):
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print(f"Usage: {sys.argv[0]} <path_to_engine_jar>")
        sys.exit(1)
    jar = sys.argv[1]
    if not os.path.isfile(jar):
        ...
        sys.exit(1)
    results = {}
    results['VULN-1'] = test_overflow(jar)
    results['VULN-2'] = test_uaf(jar)
    results['VULN-3'] = test_path_traversal(jar)
    results['VULN-4'] = test_race(jar)
    results['VULN-5'] = test_command_injection(jar)
    results['VULN-6'] = test_double_close(jar)
    results['INFO-LEAK'] = test_info_leak(jar)
    print("\nResults:")
