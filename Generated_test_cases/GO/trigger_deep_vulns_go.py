...
def test_uaf(binary):
    print("[*] Testing VULN-2: use-after-free (dangling pointer into slice)")
    config = {
        "port": 18081,
        "jobs": [{
            "type": "data_manage",
...
def test_double_close(binary):
    print("[*] Testing VULN-6: double close on file descriptor")
...
def test_info_leak(binary):
    print("[*] Testing info leak (sensitive string written to log)")
    config = {
        "port": 18086,
        "jobs": [{"type": "log_leak"}]
    }
    ...
    try:
        ...
        if os.path.exists(leak_file):
            with open(leak_file, 'rb') as f:
                data = f.read()
            if b"sensitive data" in data:
                print("    Info leak triggered (sensitive data found in log)")
                os.unlink(leak_file)
                return True
            else:
...
if __name__ == "__main__":
    ...
    results['VULN-6'] = test_double_close(binary)
