...
class SqlConcatRule(BaseRule):
    ...
    REGEX_PATTERNS = {
        "python": r"execute\s*\(\s*[^)]*\+[^)]*\)",
        "javascript": r"\.query\s*\(\s*[^)]*\+[^)]*\)",
        "java": r"(executeQuery|executeUpdate|prepareStatement)\s*\(\s*[^)]*\+[^)]*\)",
        "go": r"\.(Query|Exec|QueryRow)\s*\(\s*[^)]*\+[^)]*\)",
        "php": r"(query|exec|prepare)\s*\(\s*[^)]*\.\s*\$",
    }
    ...
    def _make_finding(self, node, lang):
        return Finding(
            ...
            message=f"Potential SQL injection via string concatenation in {lang.upper()}.",
            code_snippet="",
            cwe_id=self.cwe_id,
        )
