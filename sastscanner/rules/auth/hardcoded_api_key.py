...
class HardcodedApiKeyRule(BaseRule):
    @property
    def name(self) -> str:
        return "Hardcoded API Key / Token"
    @property
    ...
    def check(self, chunk: Dict[str, Any], context: Dict[str, Any]) -> List[Finding]:
        ...
        for node in nodes:
            ...
            for target in node.targets:
                if re.search(r'(?i)(api_key|apikey|token|access_key|secret_key)', target):
    ...
    def _create_finding(self, chunk, node, target):
        return Finding(
            ...
            message=f"Hardcoded API key / token in assignment to '{target}'",
